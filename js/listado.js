...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    if(id != null){
        restaurantId = this.restaurantes.find(restaurant => id === restaurant.id);
        return restaurantId;
    }
...
Listado.prototype.obtenerCiudades = function() {
    ...
    var ciudades = [];
    this.restaurantes.map(restaurant => ciudades.push(restaurant.ubicacion));
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    var ciudadesSinRepetidos = eliminarRepetidos(ciudades);

    return ciudadesSinRepetidos.sort();
}
...
Listado.prototype.obtenerRubros = function() {
    //En este array se van a cargar los arrays de rubros
    var rubros = []; 
    this.restaurantes.map(restaurant => rubros.push(restaurant.rubro));
    var rubrosSinRepetidos = eliminarRepetidos(rubros);
...
//funcion que utilizar obtenerRubros(), obtenerUbicaciones() y obtenerHorarios() oara eliminar repetidos de un arreglo:
function eliminarRepetidos(arregloAfiltrar) {
    var arregloFiltrado = arregloAfiltrar.filter(function(elem, index, self) {
